          ...
          <div className="flex items-center justify-between text-xs">
            ...
            <div className="flex gap-2">
              <button 
                ...
                className={`px-2 py-1 rounded-md transition-colors ${sortBy === 'date' ? 'bg-emerald-100 text-emerald-700 font-semibold' : 'text-slate-500 hover:bg-slate-100'}`}
              >
                Date
              </button>
              <button 
                onClick={() => setSortBy('name')}
                className={`px-2 py-1 rounded-md transition-colors ${sortBy === 'name' ? 'bg-emerald-100 text-emerald-700 font-semibold' : 'text-slate-500 hover:bg-slate-100'}`}
              >
                Nom
              </button>
            </div>
            ...
            filteredAndSortedConversations.map((conv) => (
              <div
                ...
                className={`p-4 border-b border-slate-50 cursor-pointer hover:bg-slate-50 transition-colors ${
                  selectedConversation?.id === conv.id ? 'bg-emerald-50/50 border-l-4 border-l-emerald-500' : 'border-l-4 border-l-transparent'
                }`}
              >
                <div className="flex justify-between items-start mb-1">
                  <h3 className={`font-medium ${selectedConversation?.id === conv.id ? 'text-emerald-700' : 'text-slate-900'}`}>
                    {conv.full_name}
                  </h3>
              ...
              {messages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.sender_id === myId ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[70%] rounded-2xl p-4 shadow-sm ${
                    ...
                    <p className={`text-[10px] mt-1 text-right ${msg.sender_id === myId ? 'text-emerald-100' : 'text-slate-400'}`}>
            ...
            <div className="p-4 border-t border-slate-100 bg-white">
              ...
              >
                <Input 
                  ...
                  className="flex-1"
                />
                <Button type="submit" icon={Send} className="bg-emerald-600 hover:bg-emerald-700">Envoyer</Button>
              </form>
            </div>
          </>
        ) : (
